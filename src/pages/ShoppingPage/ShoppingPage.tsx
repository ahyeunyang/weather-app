...
const essentials = [
  { eyebrow: '새로운 시간을 준비하는 법,', items: ['업사이클 가죽 다이어리', '포켓 가죽 다이어리', '컬러 가죽 다이어리'] },
  { eyebrow: '가볍게 챙기는 외출 준비,', items: ['포근한 데일리 머플러', '가벼운 미니 크로스백', '매일 신는 컬러 양말'] },
  { eyebrow: '실내에서도 기분 좋게,', items: ['따뜻한 무드 조명', '향기로운 패브릭 미스트', '테이블 미니 화병'] },
]
...
export function ShoppingPage() {
  ...
  const tabProducts = selectedEssential.items.map((_, index) => selectedEssential.items[(roundTab + index) % selectedEssential.items.length])
  ...
  return <main className={styles.page} id="shopping">
    ...
    <section className={styles.adBanner} aria-label="광고"><div className={styles.adImage} aria-label="광고 PNG 이미지 영역" /><h2>푸리가 먹는<br /><strong>빙수는</strong> 어디꺼지?</h2><span>AD</span></section>
    <section className={styles.section}><SectionHeading eyebrow="새로운 시간을 준비하는 법," title="2027 다이어리" /><div className={styles.listProducts}>{essentials[0].items.map((item) => <article key={item}><div aria-label="상품 이미지 영역" /><div><h3>[MAKERS ONLY]<br />{item}</h3><p><strong>76,500원</strong></p></div></article>)}</div><button className={styles.moreButton} type="button">214개 상품 더보기 <AppIcon name="chevronRight" /></button></section>
    <section className={styles.section}><header className={styles.tabbedHeading}><div><ProviderMarks count={3} active={essentialTab} onSelect={setEssentialTab} /><p>{selectedEssential.eyebrow}</p><h2>이 아이들 중 하나를 데려가렴</h2></div><button className={styles.sectionAction} type="button" aria-label="추천 카카오톡 공유"><AppIcon name="kakao" /></button></header><div className={styles.roundTabs} role="tablist" aria-label="추천 상품 종류">{selectedEssential.items.map((item, index) => <button key={item} className={roundTab === index ? styles.activeRoundTab : undefined} type="button" role="tab" aria-selected={roundTab === index} aria-controls="recommendation-products" onClick={() => setRoundTab(index)}><span aria-label="상품 이미지 영역" /><h3>{item}</h3></button>)}</div><div className={styles.tabPanel} id="recommendation-products" role="tabpanel"><div className={styles.cardSlider}>{tabProducts.map((item) => <article key={`card-${item}`}><div /><span>더드림마켓</span><h3>{item}</h3><p><strong>76,500원</strong> <del>79,500원</del></p></article>)}</div><button className={styles.centerMore} type="button">214개 상품 더보기 <AppIcon name="chevronRight" /></button></div></section>
