...
- 하단 고정 메뉴: `날씨`, `쇼핑`, `마이페이지`

현재 상품, 프로모션 이미지와 검색 아이콘은 준비 전이므로 회색 배경의 플레이스홀더로 구현했습니다. 쇼핑 화면은 제공된 `사러` 개별 화면의 정보 순서와 비율을 기준으로 구성하며, 탭을 바꾸면 날씨 질문과 상품 분류 문구가 바뀝니다. 하단 메뉴는 아래로 스크롤하면 숨고 위로 스크롤하면 다시 나타나며 최상단에서는 항상 표시됩니다. 스플래시는 약 2.2초 표시된 뒤 현재 구현 확인용 쇼핑 화면으로 전환됩니다. 실제 라우팅을 구성할 때 날씨, 쇼핑과 마이페이지 경로로 분리합니다.

상단 `사러·먹으러·놀러` 메뉴는 모두 `1.6rem`, 굵기 `650`을 사용합니다. 활성 메뉴도 같은 크기와 굵기를 유지하고 글자색만 진하게 표시합니다.

`먹으러` 탭의 대표 비주얼은 `eat-sunny-sky.png`와 `eat-mango-bingsu.png`를 별도 레이어로 사용합니다. 하늘 사진에는 프로젝트의 실제 `weather-logo-black-fixed-v2.svg`를 CSS 마스크로 적용하고, 망고 빙수는 로고 오른쪽 아래에 겹칩니다. 임의로 생성한 W 모양은 사용하지 않습니다.
...
검색, 장바구니, 하단 내비게이션, 공유와 새로고침 같은 공통 UI SVG 아이콘은 `public/icons/ui`에 보관합니다. 화면에서는 `AppIcon` 컴포넌트와 타입이 지정된 `icons.ts` 매핑을 사용하며, 현재 쇼핑 화면의 검색과 새로고침 아이콘을 연결했습니다. 파일명과 추가할 기본 아이콘 목록은 [`public/icons/ui/README.md`](./public/icons/ui/README.md)에서 관리합니다. 아직 없는 하단 내비게이션 아이콘은 동일한 크기의 플레이스홀더를 사용합니다.
